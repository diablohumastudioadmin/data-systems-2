...
static func generate_resource_class(table_name: String, properties: Array[Dictionary]) -> Error:
	var file_path = _get_resource_file_path(table_name)
...
static func regenerate_all(table_data: Array[Dictionary]) -> int:
	var count = 0
	for entry in table_data:
		if generate_resource_class(entry.table_name, entry.properties) == OK:
			count += 1
...
## Get resource file path for a table
static func _get_resource_file_path(table_name: String) -> String:
	return BASE_RESOURCES_PATH.path_join(table_name.to_lower() + ".gd")


## Check if resource class exists
static func resource_exists(table_name: String) -> bool:
	return FileAccess.file_exists(_get_resource_file_path(table_name))


## Delete resource class file
static func delete_resource_class(table_name: String) -> Error:
	var file_path = _get_resource_file_path(table_name)
