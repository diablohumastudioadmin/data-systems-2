@tool
class_name DH_VRE_ResourceList
...
var toolbar_vm: DH_VRE_ToolbarVM

var _rows: Array[DH_VRE_ResourceRow] = []
...
func _on_columns_changed(columns: Array[DH_VRE_ResourceProperty]) -> void:
...
func _on_rows_edited(resources: Array[Resource]) -> void:
	for res: Resource in resources:
		...
		if _resource_path_to_row.has(path):
			var row: DH_VRE_ResourceRow = _resource_path_to_row[path]
			if is_instance_valid(row):
				row.update_display()


func _add_row(row_vm: DH_VRE_ResourceRowVM) -> void:
