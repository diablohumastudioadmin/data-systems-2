...
static func get_diablohuma_toolbars_from_toolbar(toolbar_: PopupMenu) -> Dictionary[String, PopupMenu]:
	var inner_toolbars: Dictionary[String, PopupMenu]
	for ii in toolbar_.item_count:
		...
		if !active_toolbar.has_meta("dhs_toolbar"): continue
		inner_toolbars[toolbar_.get_item_text(ii)] = active_toolbar
