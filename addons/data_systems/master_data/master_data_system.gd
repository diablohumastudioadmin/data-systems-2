...
func load_all_instances() -> void:
	...
	for type_name in type_registry.get_master_type_names():
		load_instances(type_name)

	print("[MasterDataSystem] Loaded instances for %d types" % data_instances.size())
...
func load_instances(type_name: String) -> Error:
	...
	print("[MasterDataSystem] Loaded %d instances of %s" % [data_instances[type_name].size(), type_name])
	return OK
...
func save_instances(type_name: String) -> Error:
	...
	if error == OK:
		print("[MasterDataSystem] Saved %d instances of %s" % [instances.size(), type_name])
	else:
