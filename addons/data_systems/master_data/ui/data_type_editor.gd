...
## Allows designers to define master data and user data types

signal type_selected(type_name: String, is_user_data: bool)

@onready var type_tabs: TabBar = $VBox/TypeTabs
...
@onready var new_type_btn: Button = $VBox/HBox/TypeListPanel/VBox/NewTypeBtn
@onready var refresh_btn: Button = $VBox/HBox/TypeListPanel/VBox/RefreshBtn
...
func _setup_ui() -> void:
	type_tabs.add_tab("Master Data")
	type_tabs.add_tab("User Data")


func _connect_signals() -> void:
	type_tabs.tab_selected.connect(_on_tab_selected)
...
func _refresh_type_list() -> void:
	...
	var type_names = master_data_system.type_registry.get_user_type_names() if current_is_user_data else master_data_system.type_registry.get_master_type_names()
...
func _load_type(type_name: String) -> void:
	...
	var type_def = master_data_system.type_registry.get_type(type_name, current_is_user_data)
	if type_def == null:
...
func _add_property_row(prop_name: String = "", prop_type: DataTypeDefinition.PropertyType = DataTypeDefinition.PropertyType.STRING, default_value: Variant = null) -> void:
	var row = preload("res://addons/data_systems/master_data/ui/property_editor_row.gd").new()
...
func _on_save_type_pressed() -> void:
	...
	if current_type_name.is_empty():
		# New type
		success = master_data_system.type_registry.add_type(definition)
	else:
		# Update existing type
		success = master_data_system.type_registry.update_type(definition)

	if success:
		print("[DataTypeEditor] Saved type: %s" % type_name)
		current_type_name = type_name
		_refresh_type_list()
...
func _on_delete_type_pressed() -> void:
	...
	confirm.confirmed.connect(func():
		var success = master_data_system.type_registry.remove_type(current_type_name, current_is_user_data)
		if success:
			print("[DataTypeEditor] Deleted type: %s" % current_type_name)
			_clear_editor()
			_refresh_type_list()
		confirm.queue_free()
	)
