...
- `Database` (Resource) → `tables: Array[DataTable]` → each `DataTable` has `type_name` + `instances: Array[DataItem]`
...
- `_scan_filesystem()` after type changes
...
3. **CACHE_MODE_REPLACE** — `get_type_properties()` and `_create_data_item()` use `ResourceLoader.CACHE_MODE_REPLACE` to bypass script cache after regeneration
